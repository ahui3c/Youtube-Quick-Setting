...
Shorts 設定中的「首頁 Shorts 顯示頻道名稱」與「Shorts 顯示發布時間資訊」預設開啟。擴充功能會在首頁卡片標題下方補上可點擊的頻道名稱、在觀看次數後顯示「2 天前」等相對時間，並在 Shorts 播放頁的頻道與標題之間顯示發布時間。播放頁優先讀取 YouTube 當頁已載入的公開發布日期；首頁只為目前顯示的卡片向 YouTube 取得公開資料。查詢結果只在目前頁面快取，不會傳送到第三方伺服器，兩項功能可分別關閉。
