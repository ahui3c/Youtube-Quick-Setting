...
- 可選擇顯示 Return YouTube Dislike 提供的預估踩數（預設關閉）
...
「顯示預估踩數」預設關閉。開啟時 Chrome 會詢問 `returnyoutubedislikeapi.com` 選用權限，之後在 YouTube 原生倒讚按鈕旁顯示 `≈` 標記的第三方推算數字。資料由 [Return YouTube Dislike](https://returnyoutubedislike.com/) 提供，不是 YouTube 官方精確統計；本擴充功能只讀取數字，不會把使用者的按讚或倒讚操作回傳給第三方。關閉後會移除選用權限。
