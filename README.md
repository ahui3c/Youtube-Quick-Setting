...
- 一般影片與 Shorts 使用獨立設定
- 可在 YouTube 首頁的 Shorts 卡片補上可點擊的頻道名稱與發布時間
...
Shorts 設定中的「首頁 Shorts 顯示頻道名稱」與「首頁 Shorts 顯示發布時間」預設開啟。擴充功能只會為目前顯示的卡片向 YouTube 取得公開頻道資料與影片發布日期，在標題下方補上可點擊的頻道名稱，並在觀看次數後顯示「2 天前」等相對時間；兩項功能可分別關閉。查詢結果只在目前頁面快取，不會傳送到第三方伺服器。
