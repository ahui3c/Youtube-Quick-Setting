...
本拡張機能は、機能提供のために現在の YouTube 動画タイプ、チャンネル名、チャンネル識別情報をローカルで読み取ります。ホームのショートのチャンネル名は、ログイン Cookie なしで YouTube 自身の公開メタデータ機能から取得します。設定は Chrome の同期ストレージに保存され、開発者はアクセスできません。広告、トラッカー、解析サービス、リモートコード、開発者運営サーバーは使用しません。
