...
在 YouTube 一般影片與 Shorts 頁面，依使用者偏好自動套用播放速度及可用畫質，提供鍵盤快速操作、影片標題與網址複製、畫面提示及首頁 Shorts 頻道名稱。
...
僅在使用者點擊複製按鈕或於 YouTube 影片頁按下 S 時，把目前影片標題與公開網址寫入系統剪貼簿。本擴充功能不讀取剪貼簿內容，也不會自動複製。
...
- 網站內容：在本機暫時讀取目前影片的標題、公開網址與頻道名稱／識別資訊；標題及網址只在使用者主動操作後寫入系統剪貼簿，首頁 Shorts 公開網址只傳送回 YouTube，不傳送給開發者或其他服務
- 使用者活動：否
