...
在 YouTube 一般影片與 Shorts 頁面，依使用者的全域或頻道專屬偏好，自動套用播放速度及可用畫質，並提供鍵盤快速調速與畫面提示。
...
在 YouTube 影片與 Shorts 頁面讀取影片類型及頻道識別資訊，控制 HTML5 播放速度、呼叫 YouTube 播放器可用畫質介面、同步設定面板顯示值，並呈現套用結果提示。權限不會用於其他網站。
...
- 網站內容：在本機暫時讀取目前影片的頻道名稱／識別資訊，以選擇頻道專屬設定；不傳送給開發者或第三方
- 使用者活動：否
