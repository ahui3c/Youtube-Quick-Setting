...
- `+`／`-` で速度変更、`*` で即座に `1×` へ戻す
- `S` キーまたはパネルのメインボタンで「タイトル＋URL」をコピーし、確認表示中にもう一度 `S` を押すと Facebook 共有画面を表示
...
| `-` | 次の遅い再生速度を選択 |
| `*` | 現在の再生を `1×` に戻す |
| `S` | タイトル＋URL をコピーし、確認表示中にもう一度押すと Facebook 共有画面を表示 |
...
メインのコピーボタンと `S` キーは常に「タイトル＋URL」をコピーします。コピー成功の確認が表示されている間にもう一度 `S` を押すと、動画 URL とタイトル文字列を含む Facebook 共有画面を開きます。矢印メニューの最後にある「タイトル＋リンクを Facebook に共有」からも、同じ共有画面を直接開けます。その他の形式は一度だけコピーし、メインボタンとショートカットの既定値は変更しません。毎回、パネルまたは動画画面に成功状態と内容の要約を表示します。

最終的なタイトル、サムネイル、文字の配置は、公開 YouTube リンクをもとに Facebook が決定します。ブラウザがポップアップをブロックした場合はエラーを表示し、自動で再試行しません。
