...
- 通常動画とショートを個別に設定
- YouTube ホームのショートカードにクリック可能なチャンネル名と公開時刻を追加
...
「ホームのショートにチャンネル名を表示」と「ホームのショートに公開時刻を表示」は既定でオンです。現在表示されているカードについてのみ YouTube から公開チャンネル情報と動画の公開日を取得し、タイトルの下にクリック可能なチャンネル名、視聴回数の後に「2日前」などの相対時刻を追加します。各機能は個別に無効化できます。結果は現在のページ内だけでキャッシュされ、第三者サーバーには送信されません。
