...
- Return YouTube Dislike の低評価推定数を任意で表示（既定はオフ）
...
「低評価の推定数を表示」は既定でオフです。有効にすると `returnyoutubedislikeapi.com` への任意アクセス権限を求め、YouTube 標準の低評価ボタン横に `≈` 付きの第三者推定数を表示します。データは [Return YouTube Dislike](https://returnyoutubedislike.com/) が提供し、YouTube 公式の正確な数値ではありません。本拡張機能は数値を読み取るだけで、ユーザーの評価操作を第三者へ送信しません。無効にすると任意権限を削除します。
