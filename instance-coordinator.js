(function initYTQSInstanceCoordinator(global) {
  "use strict";

  const PRODUCT = "ahui3c.youtube-quick-settings-toolbox";
  const PROTOCOL_VERSION = 1;
  const MARKER_NAME = "ytqs-extension-instance";
  const DEVELOPMENT_DISTRIBUTION = "development";
  const CHROME_STORE_DISTRIBUTION = "chrome-web-store";
  const OFFICIAL_CHROME_EXTENSION_ID = "ababphalnahfigfafmaamnpnlbhnjnhl";

  function versionParts(value) {
    return String(value || "0").split(/[.-]/).slice(0, 4).map((part) => Number.parseInt(part, 10) || 0);
  }
  ...
  function classifyDistribution(extensionId, buildDistribution) {
    if (String(extensionId || "") === OFFICIAL_CHROME_EXTENSION_ID) return CHROME_STORE_DISTRIBUTION;
    if (String(buildDistribution || "") === CHROME_STORE_DISTRIBUTION) return DEVELOPMENT_DISTRIBUTION;
  ...
  function preferredInstance(left, right) {
    ...
    return String(left?.extensionId || "").localeCompare(String(right?.extensionId || "")) <= 0 ? left : right;
  }
  ...
  global.YTQSInstanceCoordinator = Object.freeze({
    PRODUCT,
    PROTOCOL_VERSION,
    MARKER_NAME,
